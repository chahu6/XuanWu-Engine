...
namespace YAML {
	...
	/*template<>
	struct convert<XuanWu::UUID>
	{
		static Node encode(const XuanWu::UUID& uuid)
		{
			Node node;
		...
		static bool decode(const Node& node, XuanWu::UUID& uuid)
		{
...
namespace XuanWu
{
	static std::string RigidBody2DBodyTypeToString(Rigidbody2DComponent::BodyType bodyType)
	{
		...
		XW_CORE_ASSERT(false, "Unknown body type");
		return {};
	}
	...
	{
		out << YAML::BeginMap;
		out << YAML::Key << "Entity" << YAML::Value << "213232132143";
	...
	void SceneSerializer::Serialize(const std::string_view filepath)
	{
		YAML::Emitter out;
		out << YAML::BeginMap;
	...
	bool SceneSerializer::Deserialize(const std::string_view filepath)
	{
		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				XW_CORE_TRACE("Deserialized entity with ID = {0}, name = {1}", uuid, name);
	...
	Ref<Serializer> Serializer::Create(Ref<Scene>& scene)
	{
		return CreateRef<SceneSerializer>(scene);
	}
}